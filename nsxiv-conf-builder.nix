{ pkgs, conf, ... }:
let
  toCharArray = s: if isNull s then "NULL" else ''"${s}"'';
  win_fg = ''{ ${toCharArray conf.win_fg.x_resource}, ${toCharArray conf.win_fg.value} }'';
  win_bg = ''{ ${toCharArray conf.win_bg.x_resource}, ${toCharArray conf.win_bg.value} }'';
  bar_fg = ''{ ${toCharArray conf.bar_fg.x_resource}, ${toCharArray conf.bar_fg.value} }'';
  bar_bg = ''{ ${toCharArray conf.bar_bg.x_resource}, ${toCharArray conf.bar_bg.value} }'';
in
''
...
static const char *WIN_BG[]   = ${win_bg};
static const char *WIN_FG[]   = ${win_fg};
static const char *MARK_FG[]  = { "Nsxiv.mark.foreground",      NULL };
#if HAVE_LIBFONTS
static const char *BAR_BG[]   = ${bar_bg};
static const char *BAR_FG[]   = ${bar_fg};
...
static const int thumb_sizes[] = { 32, 64, 6, 128, 160 };
...
static const button_t buttons_img[] = {
	...
	{ ControlMask,  1,                g_switch_mode,        None },
