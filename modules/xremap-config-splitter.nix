...
rec {
  ...
  # Returns: { binding, cleanAction, originalAction, description }
  processRemapEntry = key: action: let
    extracted = extractDescription action;
  in {
    binding = key;
    cleanAction = extracted.cleanAction;
    originalAction = action;  # Keep original for bindings
    description = extracted.description;
  };

  # Process a remap attribute set recursively
  # Returns: { cleanRemap, bindings }
  processRemap = remap: let
    entries = mapAttrsToList processRemapEntry remap;

    # Recursively process nested remaps in actions (for clean config)
    processedEntries = map (e:
      if isAttrs e.cleanAction && e.cleanAction ? remap then
        let
          nestedResult = processRemap e.cleanAction.remap;
        in e // {
          cleanAction = (removeAttrs e.cleanAction ["remap"]) // { remap = nestedResult.cleanRemap; };
        }
      else
        e
    ) entries;

    cleanRemap = listToAttrs (map (e: nameValuePair e.binding e.cleanAction) processedEntries);

    # For bindings, use original action to preserve descriptions for rofi-launcher
    bindings = map (e: {
