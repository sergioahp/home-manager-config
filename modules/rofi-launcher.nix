...
let
  splitter = import ./xremap-config-splitter.nix { inherit lib; };
    ...
    processBinding = binding:
      ...
      in
        if action == null then
          ...
          [{
            keySequence = fullKey;
            description = binding.description or action.description or "No description";
  ...
  makeLauncher = { config, name ? "xremap-launcher" }:
    let
      ...
      luaScript = ''
        ...
        -- Uses shell escape pattern: close quote, escaped quote, open quote
