  ...
  entries = [
    ...
    }
    {
      ...
      args = [
        ...
        "-c"
        ''
          # Freeze the screen with hyprpicker (-r renders a still frame, -z
          # drops the zoom lens) so the slurp selection and the grim capture
          # both see a frozen image. Capture to a private temp file and kill
          # hyprpicker before swappy opens, otherwise the frozen overlay would
          # linger on top of the editor (swappy -f blocks until you close it).
          ${pkgs.hyprpicker}/bin/hyprpicker -r -z &
          picker=$!
          # hyprpicker captures and commits its frozen overlay asynchronously,
          # so poll for the layer-surface instead of a blind sleep: wait at most
          # 200ms, bail if the process dies or the overlay never shows (otherwise
          # grim would grab the live desktop), and proceed the instant it is up.
          ready=
